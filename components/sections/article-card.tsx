...
export function ArticleCard({ article, featured = false }: ArticleCardProps) {
  ...
  return (
    <Link
      href={`/blogs/${article.slug}`}
      className="group relative isolate flex aspect-[5/6] w-full overflow-visible rounded-[1.35rem] text-card-foreground motion-reduce:transition-none hover:z-50 [content-visibility:auto]"
    >
      <div
      ...
      >
        ...
        <div className="absolute inset-x-0 top-0 z-20 flex items-center justify-between gap-3 p-3.5 sm:p-4">
          ...
          <span className="type-meta text-white/70">{publishedDate}</span>
        </div>
        <div
        ...
        >
          <div className="flex h-full px-3.5 pb-3.5 pt-3.5 sm:px-4 sm:pb-4 sm:pt-4">
            <div className="flex h-full max-w-[30ch] items-start">
              {article.excerpt ? (
                <p
                  className="type-body invisible max-w-[24ch] overflow-hidden text-[0.84rem] leading-[1.55] text-white/82 opacity-0 transition-[opacity,visibility] duration-300 delay-150 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:visible group-hover:opacity-100"
